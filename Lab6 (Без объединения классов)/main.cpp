...
int main ()
{
  ...
  Temp.Calculation (data) ;
  while (!MyADC::IsReady())
  {
  }; 
  data = MyADC::Get();
  Voltag.CalculationV(data);
  
  std::uint32_t Value= MyADC::Get();

  std::cout << " Temperature: " << Temp.GetValue () <<  std::endl ; ;
  std::cout <<  " Voltage: " << Voltag.GetValueV() << endl ;
  }
}
;
